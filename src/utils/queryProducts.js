class queryProducts {
  ...
  ratingQuery = () => {
    this.products = this.query.rating
      ? this.products.filter(
          (c) =>
            c.rating >= parseInt(this.query.rating) &&
  ...
  searchQuery = () => {
    this.products = this.query.searchValue
      ? this.products.filter((p) =>
          p.name.toLowerCase().includes(this.query.searchValue.toLowerCase())
        )
      : this.products;
  ...
  priceQuery = () => {
    this.products = this.products.filter(
      (p) => p.price <= +this.query.highPrice && p.price >= +this.query.lowPrice
    );
